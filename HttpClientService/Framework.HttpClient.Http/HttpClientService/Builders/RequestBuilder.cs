...
public class RequestBuilder : IRequestBuilder
{
    private readonly JsonSerializerOptions _jsonSerializerOptions = new()
    {
        ...
        TypeInfoResolver = new DefaultJsonTypeInfoResolver
        {
            Modifiers =
            {
                typeInfo =>
                {
                    if (typeof(IHttpRequest)
                        .IsAssignableFrom(typeInfo.Type))
                    {
                        var ignoredProperties = typeof(IHttpRequest)
    ...
    public HttpRequestMessage Build<TRequest>(
        TRequest request,
        string contentType)
        where TRequest : IHttpRequest
    {
        ...
        var requestUri = BuildRequestUri(
            httpRequestResolveContext.FinalRoute,
            httpRequestResolveContext.QueryParams);
        ...
        {
            if (httpRequestResolveContext.BodyContent is not null)
            {
    ...
    public HttpRequestResolverContext ResolveRequestFields<TRequest>(
        TRequest request)
        where TRequest : IHttpRequest
    {
        ...
        // Step 1: Exclude IHttpRequest props
        ...
        var propDict = props.ToDictionary(
            p => p.Name,
            p => p.Value,
        ...
        // Step 4: Resolve route params
        var usedInRoute = new HashSet<string>(
            StringComparer.OrdinalIgnoreCase);


        var finalRoute = RouteParamRegex.Replace(
            routeTemplate,
            match =>
            {
        ...
        object? bodyContent = method == HttpMethod.Post ||
        ...
        return new HttpRequestResolverContext(
            finalRoute,
            queryParams,
            bodyContent);
    }
